...
function allocateFirstEligibleProject(
  date: CivilDate,
  availableCapacity: Rational,
  states: ProjectTeamState[],
): void {
  if (isZero(availableCapacity)) return;
...
function planTeam(input: PlanningInput, team: Team): TeamPlanningResult {
  ...
  for (const date of civilDatesInclusive(
    ...
    allocateFirstEligibleProject(date, rationalOf(available), states);
  }
...
/**
 * Phase 2A sequential allocator. This deliberately temporary rule is not the
 * final admission, deadline, sharing, quantization, or redistribution policy.
 */
